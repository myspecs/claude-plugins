...
- Starting cloud sessions, writing repository configuration, or creating routines without the user's agreement in this session. Choosing the `full` autonomy level in this session is that agreement for later waves, lanes and planned pull-request splits of the current milestone; nothing else is.
...
3. Ask once, in one `AskUserQuestion` call: concurrency cap, merge method (squash or merge commit), autonomy level (`ask-each`, `merge-on-gate`, `full`; recommend `merge-on-gate`), and whether cloud dispatch is allowed. Record the answers under `policy` in `.specs/<bundle>/factory-sessions.json`; every later step reads them from there. When the user changes the level mid-run, update `policy` and save the preference to memory.
