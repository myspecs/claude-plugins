...
5. Per wave (or all lanes at once): state the cost, dispatch to the cloud (local only when the policy forbids cloud, the user asked for local, or the cloud refused), record every session at once in `.specs/<bundle>/factory-sessions.json`, arm one pull-request watch.
6. As each pull request becomes ready: verify, merge when it passes the ready-to-merge gate (asking first only at `ask-each`), watch its post-merge runs, mark `[x]` with a `Merged:` note (a lane's pull request marks every task in that lane). At `full`, when the merged work was all the next wave was waiting for and its deploy is green, dispatch that wave at once and report it.
