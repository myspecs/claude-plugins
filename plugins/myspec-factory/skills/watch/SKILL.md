...
| `board-tick.sh [minutes] [ticks]` | nothing: it prints `board-tick <n>/<ticks>` every few minutes (default 5) so the manager reads the factory board, which wakes nobody when a worker writes it. Run one while any worker runs and the run has a board; stop it with TaskStop when none does | after the last tick (default 6, 30 minutes at 5 minutes, the longest Monitor window): re-arm on exit or on the Monitor's expiry while workers still run |
