...
2. Acceptance criteria: for each criterion on the task and its `_Requirements:_` entries, find the test that names it (`grep -riE 'FR[-_]?003'` style) in the diff. Missing tests are a failure unless the report explains why the criterion cannot be tested automatically.
   Delegate this to a read-only subagent working in a throwaway worktree (`git worktree add --detach <scratch> <sha>`, removed afterwards) so the user's checkout is untouched: ask it to install as CI does, run each project's tests, lint and typecheck, and return a criterion-by-criterion table with `file:line` or test-name evidence. A failing test is only a finding once it has been re-run on the base commit — sandbox-only failures (missing binaries, temp-path assumptions) are pre-existing and must be reported as such. Verify the commit the pull request is actually on; a cloud worker may push more commits while the check runs.
