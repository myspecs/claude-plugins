...
Cloud workers may have pushed a branch without a pull request: `git fetch --prune` and `git branch -r --list 'origin/claude/*' 'origin/factory/*'`; for a branch whose worker reported its task numbers but no pull request exists, open one with `gh pr create --head <branch> --title "task N1, N2, …: <summary>" --body-file <report>` (`task N: <title>` for a single task) using the report from the worker's final message. Then `gh pr list --state open --json number,title,headRefName,labels,isDraft,reviewDecision,statusCheckRollup,body` and match each pull request to its tasks by branch `factory/<bundle>/tasks-<N1>-<N2>-…` or `factory/<bundle>/task-<N>`, or by title `task N1, N2, …:` or `task N:` (cloud sessions push `claude/`-prefixed branches, so the title match matters). A group pull request covers every task its title lists, and a lane pull request (branch `factory/<bundle>/lane-<L>` or title `lane L:`) every task the `## Branch Plan` lists for that lane: verify each of them, and merge or hold the pull request as a whole. Read the `## Factory report`: the end of the description, and any later pull request comment whose first line is `## Factory report` (`gh pr view <n> --json body,comments`; the newest one is current). Workers report on the pull request only, never as a MySpec attachment. A missing report means the worker did not finish; treat as failed. A draft pull request or a report whose first line starts with `BLOCKED:` is blocked, not ready.
...
4. Scope: the diff touches only the modules and files its tasks named plus tests. Unrelated changes are reported back and the pull request is not merged until the worker removes them. For a lane, the diff also stays inside the lane's `Owns` list; any path from another lane is a finding, and each shared contract is compared field by field with the other lanes' branches.
