...
2. Integrate: list open pull requests with `gh pr list --state open --json number,title,headRefName,labels,isDraft,reviewDecision,statusCheckRollup,body`; a worker pull request has a branch factory/<bundle>/tasks-<N1>-<N2>-… or factory/<bundle>/task-<N>, or a title starting "task <N1>, <N2>, …:" or "task <N>:", and covers every task its title lists; a lane pull request has a branch factory/<bundle>/lane-<L> or a title starting "lane <L>:" and covers every task listed for that lane under "## Branch Plan" in tasks.md. Skip drafts and reports whose first line starts with "BLOCKED:" (list them in the report). For each remaining pull request with green required checks on its current head, an approval from the gating reviewer on that same head commit (an approval on an earlier commit does not count; request the review again instead), no unresolved review threads, and a "## Factory report", verify tests named after the acceptance criteria exist and the constitution is respected. With policy squash: first check the tested tree is the merged tree (if the default branch gained commits touching the pull request's projects since its checks ran, run `gh pr update-branch <n>` and leave it for the next shift), then merge ONE pull request with `gh pr merge <n> --squash --match-head-commit <the head you verified>`, then wait until every workflow run on the default branch for that merge commit has completed (and stop the shift if any failed) before merging the next; then, when MySpec tools exist, mark every task the pull request covers (for a lane, every task of the lane) "[x]" in tasks.md using get_spec_file for content_version and update_spec_file with expected_version, adding "  - Merged: PR #<n>" under each task. With policy report only: list what is mergeable and do not merge.
3. Plan: if tasks.md has a "## Branch Plan" section, the wave is every lane that has no open or merged pull request. Otherwise derive ready tasks (unchecked, every _Dependencies_ done, not in flight) and group related tasks into one worker each, as many as a group holds: a dependency chain (a waiting task joins when all its unmet dependencies are in the same group), tasks that share a module or file, and tasks for the same requirement go together; a task gets a worker of its own only when nothing relates to it. Tasks that share a module or file never go to two workers. Keep groups within the current milestone and inside any split tasks.md declares. The wave is at most <N> workers; when there are more groups, merge the smallest ones.
4. Dispatch: for each group (or lane), start one worker session with a self-contained brief (first line "factory <bundle> tasks <N1>, <N2>, …: <summary>", every task block in order, cited requirements, binding constitution sections, solution excerpts, branch factory/<bundle>/tasks-<N1>-<N2>-…, one commit per task, PR title "task <N1>, <N2>, …: <summary>", Factory report at the end of the PR body, BLOCKED protocol saying for each task whether it blocks the tasks after it; a single task uses "task <N>" in the first line, branch and title). A lane brief carries every task of the lane in order, the lane's Owns and Does not touch lists, the shared contracts in full, branch factory/<bundle>/lane-<L>, one commit per task, and PR title "lane <L>: tasks <N1>, <N2>". Tell each worker to request review from <reviewer> as soon as its pull request exists, to turn on Auto-fix, and never to merge it. Use the Agent tool with isolation "remote" if available, otherwise `cd` into the repository checkout and run `claude --cloud` under a pseudo-terminal (`script -q <file> claude --cloud "$(cat <brief>)" --permission-mode auto </dev/null`; it exits 1 without a TTY). Capture each session id from the "Created cloud session:" / "View:" lines (or the agent id) immediately; expect the cloud to rewrite the session title and to push its own `claude/`-prefixed branch. If neither path works from this session, do not dispatch; report the ready wave instead.
5. Report: write a summary (merged, dispatched with a table of tasks, session id, URL, and branch; blocked; spec questions; milestone status; anything deferred) as a comment on the tracking issue #<n> with `gh issue comment` (or open a new issue titled "factory shift <date>"), and stop. The next shift reads the latest comment first to find running sessions and steers them with `claude -p "<message>" --cloud <session_id>` before dispatching new ones.
```
