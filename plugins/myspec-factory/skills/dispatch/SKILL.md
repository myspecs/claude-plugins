...
5. With a board, keep one `board-tick.sh` Monitor running while a worker is working (the `watch` skill), read the board on every line it prints, and stop it with TaskStop as soon as no worker is working.
