...
- The manager is the only writer of `tasks.md`; workers report through pull requests (`factory/<bundle>/tasks-<N1>-<N2>-…` branches and `task N1, N2, …:` titles for a group, `factory/<bundle>/task-<N>` and `task N:` for a single task, a `## Factory report` in the body).
- One worker per group of related tasks (a dependency chain, tasks sharing a module or file, the same requirement), as many as the group holds; a single-task worker only when nothing relates to the task. Groups stay inside the current milestone and any split `tasks.md` declares; more groups than the concurrency cap are merged, smallest first. The rules live in the dispatch skill's §0; `plan` and `shift` apply them.
...
- Dispatch order: Agent tool `isolation: "remote"`, `claude --cloud`, Agent tool `isolation: "worktree"`, `claude --bg --worktree`. Verify availability; do not assume. Cloud always comes first; the local paths are a fallback only when the policy forbids cloud, the user asks for local, or the cloud refuses for a lasting reason (reasons in `cloud-vs-local.md`).
